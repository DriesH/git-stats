...
**Architecture:** Three layers with strict separation — git I/O (`git2`), pure analyzers (`&[CommitRecord]` → result structs), and rendering (`ratatui`). The expensive per-commit diff pass runs in parallel via `rayon`. A loading TUI shows progress/ETA and supports graceful cancel while collection runs on rayon worker threads.

**Tech Stack:** clap (derive), git2, ratatui + crossterm, rayon, anyhow + thiserror, chrono.
...
fn cancel_returns_early() {
    ...
    assert!(records.len() < oids.len() || oids.is_empty() || records.is_empty());
}
```
...
use git2::{Commit, Diff, DiffOptions, Oid, Repository, Sort};
use rayon::prelude::*;
use crate::model::{CommitRecord, FileChurn};
...
/// Walk history newest-first (first-parent), apply --limit/--since, return Oids.
pub fn list_oids(repo: &Repository, opts: &CollectOpts) -> anyhow::Result<Vec<Oid>> {
    ...
    for oid in walk {
        ...
        if let Some(since) = opts.since {
            if commit.time().seconds() < since { continue; }
        }
        out.push(oid);
...
/// `done` is bumped per finished commit; `cancel` short-circuits the pass.
...
use Style as _; // keep import if unused in some builds
```
...
Note: `args.no_color` — ratatui colors render normally; honoring `--no-color` can be a follow-up (set a global flag the widgets read). For v1 the flag parses and is accepted; wiring it to strip styles is deferred. If the reviewer wants it enforced now, add a `OnceLock<bool>` in `theme.rs` and gate the `*_style()` fns on it.
...
| anyhow/thiserror errors | 17 + modules |

Deferred (noted YAGNI, not silent): relative `--since` phrases; enforcing `--no-color` styling; `--jobs` flag.
```
